...
const FundingEscrow = ({t, isBuyer, isActive, isDone, action, tokenAmount, tokenSymbol, feePercent, feeAmount, enoughBalance, disabled}) => (
  <Row className="mt-4">
    <Col xs="1">
      {!isDone && <RoundedIcon size="xs" image={FundIcon} bgColor={isActive ? 'primary' : 'grey'}/>}
    ...
    <Col xs={isActive ? '4' : '2'} sm={isActive ? '3' : '2'} md={isActive ? '2' : '2'}>
      ...
      {!isDone && !isActive && !isBuyer && <p className="text-muted text-small">{t('general.you')}</p>}

      {!isDone && isBuyer && <div className="bg-dark rounded p-2 position-relative">
